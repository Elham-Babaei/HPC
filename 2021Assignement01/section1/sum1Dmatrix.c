...
int main(int argc, char* argv[])      //command line arguments
{
  ...
  int dim[1],period[1];
  int my_coords[1];
  int dims[1]= {0};    // Ask MPI to decompose our processes in a 3D cartesian grid for us
  int periods[1] = {true};    // Make all dimensions periodic
  ...
  MPI_Dims_create(size, 1, dims);                  // Ask MPI to decompose our processes in a 3D cartesian grid for us
  MPI_Cart_create(MPI_COMM_WORLD, 1, dims, periods, reorder, &new_comm);         // Create a communicator given the 3D torus topology.
  MPI_Comm_rank(new_comm, &my_rank);                         // Get my rank in new communicator
  MPI_Cart_coords(new_comm, my_rank, 1, my_coords);          // Get my coordinates in the new communicator

  printf("[MPI process, old %d, new %d] I am located at (%d).\n", old_rank, my_rank, my_coords[0]);
